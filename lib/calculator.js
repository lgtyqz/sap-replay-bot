...
function parseReplayForCalculator(battleJson, buildModel) {
  ...
  // Helper: robustly extract numeric values from board JSON using explicit
  // keys or fallback hint-based detection when field names vary between
  // replays/versions.
  const getNumericFromBoard = (board, explicitKeys = [], hints = [], defaultValue = 0) => {
    if (!board || typeof board !== 'object') return defaultValue;
    ...
    for (const [key, value] of Object.entries(board)) {
      ...
      if (normalizedHints.some(h => kn.includes(h))) {
        console.debug(`[calculator] Found fallback key '${key}' -> ${n}`);
        return n;
      }
    }
    return defaultValue;
  ...
  return {
    ...
    turn: getNumericFromBoard(userBoard, ['Tur', 'Turn'], ['tur', 'turn'], 1),
    playerGoldSpent: getNumericFromBoard(userBoard, ['GoSp', 'GSp', 'GoldSpent'], ['gosp', 'gold', 'spent'], 10),
    opponentGoldSpent: getNumericFromBoard(opponentBoard, ['GoSp', 'GSp', 'GoldSpent'], ['gosp', 'gold', 'spent'], 10),
    playerRollAmount: getNumericFromBoard(userBoard, ['Rold', 'Rols', 'Rolls'], ['rol', 'roll'], 4),
    opponentRollAmount: getNumericFromBoard(opponentBoard, ['Rold', 'Rols', 'Rolls'], ['rol', 'roll'], 4),
    playerSummonedAmount: getNumericFromBoard(userBoard, ['MiSu', 'Misu', 'Summoned'], ['misu', 'summon'], 0),
    opponentSummonedAmount: getNumericFromBoard(opponentBoard, ['MiSu', 'Misu', 'Summoned'], ['misu', 'summon'], 0),
    playerLevel3Sold: getNumericFromBoard(userBoard, ['MSFL', 'M3SL', 'Level3Sold'], ['msfl', 'level3', 'l3', 'sold'], 0),
    opponentLevel3Sold: getNumericFromBoard(opponentBoard, ['MSFL', 'M3SL', 'Level3Sold'], ['msfl', 'level3', 'l3', 'sold'], 0),
    playerTransformationAmount: getNumericFromBoard(userBoard, ['TrTT', 'TrTt', 'Transformations'], ['trtt', 'transf', 'transform'], 0),
    opponentTransformationAmount: getNumericFromBoard(opponentBoard, ['TrTT', 'TrTt', 'Transformations'], ['trtt', 'transf', 'transform'], 0),
