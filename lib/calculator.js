...
const { PACK_MAP, BATTLE_OUTCOMES } = require('./config');
...
function patchSapCalculatorLogService() {
  const filePath = require.resolve('sap-calculator/simulation/dist/index.js');
  let content;
  try {
    content = fs.readFileSync(filePath, 'utf8');
  } catch (error) {
    console.warn('Failed to read sap-calculator simulation file for patching:', error);
    return;
  }
  if (content.includes('this.logService?.createLog')) {
    return;
  }
  const patched = content.replaceAll('this.logService.createLog(', 'this.logService?.createLog(');
  try {
    fs.writeFileSync(filePath, patched, 'utf8');
  } catch (error) {
    console.warn('Failed to write sap-calculator simulation patch:', error);
  }
}
function runSimulationSafe(config) {
  patchSapCalculatorLogService();
  clearCalculatorCache();
  const { runSimulation } = require('sap-calculator/simulation/dist/index.js');
  return runSimulation(config);
}
async function buildWinPercentReportHeadless(battleJsonList, buildModel) {
  const results = [];

  const isSuspiciousResult = (result, total, battleJson) => {
    if (!result || !Number.isFinite(result.playerWins) || !Number.isFinite(result.opponentWins) || !Number.isFinite(result.draws)) {
      return true;
    }
    if (result.playerWins + result.opponentWins + result.draws !== total) {
      return true;
    }
    const outcome = battleJson?.Outcome;
    if (outcome === BATTLE_OUTCOMES.WIN && result.playerWins === 0) {
      return true;
    }
    if (outcome === BATTLE_OUTCOMES.LOSS && result.opponentWins === 0) {
      return true;
    }
    if (outcome === BATTLE_OUTCOMES.TIE && result.draws === 0) {
      return true;
    }
    return false;
  };
  ...
  for (let i = 0; i < battleJsonList.length; i++) {
    const battleJson = battleJsonList[i];
    ...
    let result = null;
    try {
      result = runSimulationSafe(config);
      if (isSuspiciousResult(result, config.simulationCount, battleJson)) {
        results.push({
          line: `Turn ${i + 1}: Suspicious`,
          player: null,
          opponent: null,
          draw: null
        });
      ...
      console.error(`Failed to calculate win% for turn ${i + 1} via headless sim:`, error);
      results.push({
...
async function getWinPercentText(page, calculatorLink) {
  ...
  const resultLocator = page.getByText(/Player Wins|Opponent Wins|Draws/i).first();
  if (await resultLocator.isVisible().catch(() => false)) {
    return parseWinPercentText(await resultLocator.evaluate((node) => node?.parentElement?.textContent || node?.textContent || ''));
  }
  ...
  if (await countInput.isVisible().catch(() => false)) {
    ...
    if (await numberInput.first().isVisible().catch(() => false)) {
      await numberInput.first().fill('50');
    }
  }
...
function getDebugBattle(battleJson, buildModel) {
  ...
  const result = runSimulationSafe(config);
